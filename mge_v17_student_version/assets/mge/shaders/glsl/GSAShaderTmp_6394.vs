...
void main( void ) {
    ...
    vec3 vertexNormal_cameraspace = (MVMatrix * vec4(normalize(normal),0)).xyz;
    vec3 vertexTangent_cameraspace = (MVMatrix * vec4(normalize(tangent),0)).xyz;
    vec3 vertexBitangent_cameraspace = (MVMatrix * vec4(normalize(bitangent),0)).xyz;
    ...
    EyeDirection_cameraspace = vec3(0,0,0) - ( MVMatrix * vec4(vertex,1)).xyz;
